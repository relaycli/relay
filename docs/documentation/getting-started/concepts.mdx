...
<Note>Why read this? You don’t need to memorize every RFC to use Invoxflow, but a 5-minute tour of the landscape helps you debug faster — and impress friends at parties.</Note>
...
| Invoxflow | - | (open-core) | Sits between MUA and IMAP, giving dev-friendly JSON + webhooks. |
...
Invoxflow treats every provider the same as long as it offers IMAP/SMTP. Your code talks to a single /messages endpoint.
...
| Interaction APIs | Invoxflow | Read, search, tag, reply, build full apps. |

Use both together: Resend for blasts, Invoxflow for smart follow-ups.

## Key Terminology in Invoxflow
